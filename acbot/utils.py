...
async def asset_channel(
    ...
    *,
    ...
    if _folder:
        if _folder != "AuthorBot":
            raise NotImplementedError

        folders = await client(GetDialogFiltersRequest())

        try:
            folder = next(folder for folder in folders if folder.title == "AuthorBot")
        except Exception:
            folder = None
...
def get_git_hash() -> typing.Union[str, bool]:
    """
    Get current AuthorBot git hash
...
def get_commit_url() -> str:
    """
    Get current AuthorBot git commit url
